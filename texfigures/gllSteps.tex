\newcommand{\nodeDistance}{1.8cm}
\begin{table}
    \caption{The steps of the GLL-based CFPQ executed on the graph represented in figure~\ref{fig:input-graph} and the query represented in figure~\ref{fig:example-rsm}.}
                ...
                \resizebox{8\textwidth}{!}{
                    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=\nodeDistance, semithick]
                        ...
                        \path (B) edge         node[right]{$q_5, \epsilon$} (A)
                                (B) edge  [loop below] node[below]{$q_2, R^{q_0,v_0}_{q_1,v_0}$} (B) 
                                ;
                    \end{tikzpicture}
                }
            \end{minipage}
