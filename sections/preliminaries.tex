...
Both regular expressions and finite automata specify regular languages and can be converted into each other. Every regular language can be specified using deterministic finite automata without $\varepsilon$-transitions. Note that regular languages form a strong subset of context-free languages.
...
\end{definition}
 ...
 For simplicity, $S \to w_1 \mid w_2$ is used instead of $S \to w_1; S \to w_2$. Some example grammars are presented in equations~\ref{eqn:g_1},~\ref{eqn:g_2}, and~\ref{eqn:geo}. 
...
We use a generalization of a \textit{parse tree} to represent the result of a query.
...
\begin{definition}
    ...
    \begin{itemize}
        ...
        \item Let $w_0$ is an ordered concatenation of child's labels of node labelled with $N_i$. Then $w_0 \in \mathcal{L}(r)$, where $N_i \to r \in P$.  
    \end{itemize}
\end{definition}
